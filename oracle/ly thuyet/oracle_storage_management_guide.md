...
### Hướng dẫn các giải pháp thu hồi dung lượng (Space Reclamation) và Reset HWM:
**Tại sao lệnh DELETE không thu hồi được dung lượng?**
Khi bạn sử dụng lệnh `DELETE`, hệ thống chỉ xóa dữ liệu bên trong các khối (blocks) để lấy chỗ trống chèn dữ liệu sau này, nhưng **để lại tất cả không gian được phân bổ cho bảng**. Mốc High Water Mark (HWM - Mốc đánh dấu trần dữ liệu cao nhất của bảng) **vẫn giữ nguyên không thay đổi**. Do đó, để giải phóng dung lượng vật lý trả lại cho Tablespace, bạn cần áp dụng các giải pháp dưới đây:

#### 1. Giải pháp dùng lệnh SHRINK SPACE (Khuyên dùng)
Đây là tính năng Online Segment Shrink (từ Oracle 10g), thu nhỏ trực tuyến để khôi phục không gian và hạ mốc HWM.
...
```sql
-- Cách 1: Thu hồi toàn diện, hạ HWM (Có thể gây lock bảng trong một khoảnh khắc rất nhỏ ở bước cuối)
ALTER TABLE my_table SHRINK SPACE;

-- Cách 2: Chỉ dồn dữ liệu lại sát nhau, NHƯNG CHƯA hạ mức HWM xuống (Dùng được trong giờ cao điểm vì không gây lock)
ALTER TABLE my_table SHRINK SPACE COMPACT;

-- Cách 3: Thu hồi không gian cho cả các đối tượng phụ thuộc (cascade)
...
#### 2. Giải pháp dùng lệnh ALTER TABLE MOVE
Di chuyển phần dữ liệu thực tế của bảng sang phân vùng/khối mới, vứt bỏ đi các không gian trống và reset lại HWM.
*   **Lưu ý:** Việc di chuyển sẽ làm cho các Index bị vô hiệu hoá (Unusable). Bạn **bắt buộc phải cấu trúc lại (rebuild) toàn bộ Index** sau khi move.
```sql
ALTER TABLE my_table MOVE;
-- Hoặc move sang tablespace khác: ALTER TABLE my_table MOVE TABLESPACE new_tbs;

ALTER INDEX my_index REBUILD;
```

#### 3. Giải pháp Export / Truncate / Import
*   Dùng **DataPump (expdp)** để export phần dữ liệu còn lại.
*   Dùng **TRUNCATE TABLE** để làm sạch bảng. Lệnh Truncate (với tùy chọn mặc định `DROP STORAGE`) sẽ giải phóng tất cả không gian được phân bổ và kéo HWM về mức đầu tiên.
*   Dùng **DataPump (impdp)** để đổ lại dữ liệu vào.

#### 4. Các giải pháp nâng cao khác
*   **DBMS_REDEFINITION:** Cấu trúc lại (reorg) bảng trực tuyến, thu hồi không gian mà không gây downtime cho ứng dụng.
*   **Oracle GoldenGate / DataGuard:** Đồng bộ dữ liệu đã làm sạch sang một CSDL/node mới.

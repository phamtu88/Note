...
### 0. Xử lý Firewall và SELinux (Thao tác bằng `root`)
Để tránh các lỗi về kết nối Listener và phân quyền hệ thống, ta cần tắt Firewall và đưa SELinux về chế độ Disabled.
```bash
# 1. Tắt và dừng Firewall
systemctl stop firewalld
systemctl disable firewalld

# 2. Đưa SELinux về chế độ Disabled (Yêu cầu khởi động lại máy để áp dụng vĩnh viễn)
setenforce 0
